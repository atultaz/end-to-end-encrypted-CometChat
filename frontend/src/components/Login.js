...
const Login = (props) => {
  const { user, setUser, setIsLoading, cometChat, setEThree } =
    useContext(Context);
    ...
    if (eThree) {
      try {
        ...
        if (err.name === 'IdentityAlreadyExistsError') {
          ...
          console.log(
            `${user.fullname} has local private key: ${hasLocalPrivateKey}`
          );
    ...
    <div className='login__container'>
      ...
      <div className='login__form-container'>
        <div className='login__form'>
          ...
          <span className='login__signup' onClick={showModal}>
            Create New Account
          </span>
        </div>
      </div>
